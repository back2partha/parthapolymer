import{IronA11yKeysBehavior,IronControlState,dom,IronResizableBehavior,useShadow,Polymer,html$1 as html,addListener,findOriginalTarget,IronMultiSelectableBehaviorImpl,IronMultiSelectableBehavior,IronSelectableBehavior,IronButtonState,IronFormElementBehavior,IronValidatableBehavior,LegacyElementMixin,wrap,PolymerElement,html as html$1}from"./my-app.js";const IronFitBehavior={properties:{/**
...
// align setting is ok.
if(0>diff||0===diff&&(vAlignOk||hAlignOk)){position=candidate}}return position}};var ironFitBehavior={IronFitBehavior:IronFitBehavior},p=Element.prototype,matches=p.matches||p.matchesSelector||p.mozMatchesSelector||p.msMatchesSelector||p.oMatchesSelector||p.webkitMatchesSelector;class IronFocusablesHelperClass{/**
...
return noLabelFloat?-4:8},/**
   ...
   */_getValidity:function(_value){return this.disabled||!this.required||this.required&&!!this.value},_openedChanged:function(){var openState=this.opened?"true":"false",e=this.contentElement;if(e){e.setAttribute("aria-expanded",openState)}}});Polymer({_template:html`
    <style>
      :host {
        display: block;
        padding: 8px 0;
    ...
    <slot></slot>
`,is:"paper-listbox",behaviors:[IronMenuBehavior],/** @private */hostAttributes:{role:"listbox"}});class MyRegister extends PolymerElement{static get template(){return html$1`
...
logout(){this.set("route.path","/login")}}window.customElements.define("my-register",MyRegister);export{ironFitBehavior as $ironFitBehavior,ironFocusablesHelper as $ironFocusablesHelper,ironMenuBehavior as $ironMenuBehavior,ironOverlayBehavior as $ironOverlayBehavior,ironOverlayManager as $ironOverlayManager,ironScrollManager as $ironScrollManager,neonAnimatableBehavior as $neonAnimatableBehavior,neonAnimationBehavior as $neonAnimationBehavior,neonAnimationRunnerBehavior as $neonAnimationRunnerBehavior,paperMenuButton as $paperMenuButton,IronFitBehavior,IronFocusablesHelper,IronMenuBehavior,IronMenuBehaviorImpl,IronOverlayBehavior,IronOverlayBehaviorImpl,IronOverlayManager,IronOverlayManagerClass,NeonAnimatableBehavior,NeonAnimationBehavior,NeonAnimationRunnerBehavior,NeonAnimationRunnerBehaviorImpl,PaperMenuButton,_boundScrollHandler,_composedTreeContains,_getScrollInfo,_getScrollableNodes,_getScrollingNode,_hasCachedLockedElement,_hasCachedUnlockedElement,_lockScrollInteractions,_lockedElementCache,_lockingElements,_scrollInteractionHandler,_shouldPreventScrolling,_unlockScrollInteractions,_unlockedElementCache,currentLockingElement,elementIsScrollLocked,pushScrollLock,removeScrollLock};