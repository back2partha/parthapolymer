...
// e.g. in Firefox `<div contenteditable>` has `tabIndex = -1`
var ati=Math.max(a.tabIndex,0),bti=Math.max(b.tabIndex,0);return 0===ati||0===bti?bti>ati:ati>bti}}]);return IronFocusablesHelperClass}(),IronFocusablesHelper=new IronFocusablesHelperClass;_exports.IronFocusablesHelper=IronFocusablesHelper;var ironFocusablesHelper={IronFocusablesHelper:IronFocusablesHelper};_exports.$ironFocusablesHelper=ironFocusablesHelper;(0,_myApp.Polymer)({/** @override */_template:(0,_myApp.html$1)(_templateObject_395c5060825e11eb9753edf83d0367e1()),is:"iron-overlay-backdrop",properties:{/**
...
this.__openedRaf=null},/** @override */attached:function attached(){this.opened&&this._openedChanged(this.opened)},/**
   ...
   */prepare:function prepare(){if(this.opened&&!this.parentNode){(0,_myApp.dom)(document.body).appendChild(this)}},/**
   * Shows the backdrop.
   */open:function open(){this.opened=!0},/**
   * Hides the backdrop.
   */close:function close(){this.opened=!1},/**
   * Removes the backdrop from document body if needed.
   */complete:function complete(){if(!this.opened&&this.parentNode===document.body){(0,_myApp.dom)(this.parentNode).removeChild(this)}},_onTransitionend:function _onTransitionend(event){if(event&&event.target===this){this.complete()}},/**
...
if(z1===z1){z=z1}}return z}/**
     ...
     * @param {!Event} event
     * @private
     */},{key:"_onCaptureClick",value:function _onCaptureClick(event){var i=this._overlays.length-1;if(-1===i)return;var path=/** @type {!Array<!EventTarget>} */(0,_myApp.dom)(event).path,overlay;// Check if clicked outside of overlay.
while((overlay=/** @type {?} */this._overlays[i])&&this._overlayInPath(path)!==overlay){overlay._onCaptureClick(event);if(overlay.allowClickThrough){i--}else{break}}}/**
     ...
     * @param {!Event} event
     * @private
     */},{key:"_onCaptureKeyDown",value:function _onCaptureKeyDown(event){var overlay=/** @type {?} */this.currentOverlay();if(overlay){if(_myApp.IronA11yKeysBehavior.keyboardEventMatchesKeys(event,"esc")){overlay._onCaptureEsc(event)}else if(_myApp.IronA11yKeysBehavior.keyboardEventMatchesKeys(event,"tab")){overlay._onCaptureTab(event)}}}/**
...
if(this.opened){this._openedChanged(this.opened)}this._observer=(0,_myApp.dom)(this).observeNodes(this._onNodesChange)},/** @override */detached:function detached(){// TODO(bicknellr): Per spec, checking `this._observer` should never be
...
var activeElement=this._manager.deepActiveElement;if(activeElement===document.body||composedContains(this,activeElement)){this.__restoreFocusNode.focus()}}this.__restoreFocusNode=null;this._focusNode.blur();this._focusedChild=null}},/**
   ...
   * @param {!Event} event
   * @protected
   */_onCaptureFocus:function _onCaptureFocus(event){if(!this.withBackdrop){return}var path=(0,_myApp.dom)(event).path;if(-1===path.indexOf(this)){event.stopPropagation();this._applyFocus()}else{this._focusedChild=/** @type {Node} */path[0]}},/**
...
for(var nodes=(0,_myApp.dom)(this.$.content).getDistributedNodes(),i=0,l=nodes.length;i<l;i++){if(nodes[i].nodeType===Node.ELEMENT_NODE){return nodes[i]}}},ready:function ready(){// Ensure scrollAction is set.
...
if(!this._readied){return}if(!allowOutsideScroll){this.scrollAction="lock"}else if(!this.scrollAction||"lock"===this.scrollAction){this.scrollAction="refit"}},/**
   ...
   */_applyFocus:function _applyFocus(){var focusTarget=this.focusTarget||this.containedElement;if(focusTarget&&this.opened&&!this.noAutoFocus){focusTarget.focus()}else{IronOverlayBehaviorImpl._applyFocus.apply(this,arguments)}}});var IronMenuBehaviorImpl={properties:{/**
...
if((0,_myApp.dom)(owner).activeElement==item){return}}}},/**
   ...
   */_applySelection:function _applySelection(item,isSelected){if(isSelected){item.setAttribute("aria-selected","true")}else{item.setAttribute("aria-selected","false")}_myApp.IronSelectableBehavior._applySelection.apply(this,arguments)},/**
...
_activateHandler:function _activateHandler(event){_myApp.IronSelectableBehavior._activateHandler.call(this,event);event.stopPropagation()},/**
...
this._dropdownContent=this.contentElement;this.fire("paper-dropdown-open")}else if(null!=oldOpened){this.fire("paper-dropdown-close")}},/**
   ...
   */_disabledChanged:function _disabledChanged(disabled){_myApp.IronControlState._disabledChanged.apply(this,arguments);if(disabled&&this.opened){this.close()}},__onIronOverlayCanceled:function __onIronOverlayCanceled(event){var uiEvent=event.detail,trigger=this.$.trigger,path=(0,_myApp.dom)(uiEvent).path;if(-1<path.indexOf(trigger)){event.preventDefault()}}});/**
...
_exports.$paperMenuButton=paperMenuButton;var LegacyPolymerElementBase=(0,_myApp.LegacyElementMixin)(HTMLElement);/**
                                                                  Material design: [Dropdown
                                                                  ...
                                                                  @demo demo/index.html
                                                                  */(0,_myApp.Polymer)({/** @override */_template:(0,_myApp.html$1)(_templateObject4_395c5060825e11eb9753edf83d0367e1()),is:"paper-dropdown-menu",behaviors:[_myApp.IronButtonState,_myApp.IronControlState,_myApp.IronFormElementBehavior,_myApp.IronValidatableBehavior],properties:{/**
   ...
   * @override
   */_attachDom:function _attachDom(dom){var wrappedThis=(0,_myApp.wrap)(this);wrappedThis.attachShadow({mode:"open",delegatesFocus:!0,shadyUpgradeFragment:dom});wrappedThis.shadowRoot.appendChild(dom);return LegacyPolymerElementBase.prototype._attachDom.call(this,dom)},/** @override */focus:function focus(){// When using Shady DOM and in browsers that don't support
...
for(var nodes=(0,_myApp.dom)(this.$.content).getDistributedNodes(),i=0,l=nodes.length;i<l;i++){if(nodes[i].nodeType===Node.ELEMENT_NODE){return nodes[i]}}},/**
   ...
   */_onTap:function _onTap(event){if((0,_myApp.findOriginalTarget)(event)===this){this.open()}},/**
...
return noLabelFloat?-4:8},/**
   ...
   */_getValidity:function _getValidity(_value){return this.disabled||!this.required||this.required&&!!this.value},_openedChanged:function _openedChanged(){var openState=this.opened?"true":"false",e=this.contentElement;if(e){e.setAttribute("aria-expanded",openState)}}});(0,_myApp.Polymer)({_template:(0,_myApp.html$1)(_templateObject5_395c5060825e11eb9753edf83d0367e1()),is:"paper-listbox",behaviors:[IronMenuBehavior],/** @private */hostAttributes:{role:"listbox"}});var MyRegister=/*#__PURE__*/function(_PolymerElement){babelHelpers.inherits(MyRegister,_PolymerElement);function MyRegister(){babelHelpers.classCallCheck(this,MyRegister);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(MyRegister).apply(this,arguments))}babelHelpers.createClass(MyRegister,[{key:"addpatient",//setting type values
value:function addpatient(){var vals={name:this.$.fname.value,address:this.$.address.value,email:this.$.email.value,io:this.$.io.value,doctr:this.$.doctr.value,number:this.$.phone.value},pInfo=[];//getting values from patient form
if(this.patientInfo){pInfo=[].concat(babelHelpers.toConsumableArray(this.patientInfo),[vals]);//storing patientInfo values to pInfo
}else{pInfo=[vals]}this.set("patientInfo",pInfo)}},{key:"removeUser",value:function removeUser(){var user="wing sang",index=this.patientInfo.indexOf(user);this.splice("patientInfo",index,1)}//removes user from the list
},{key:"logout",value:function logout(){this.set("route.path","/login")}}],[{key:"template",get:function get(){return(0,_myApp.html)(_templateObject6_395c5060825e11eb9753edf83d0367e1())}},{key:"properties",get:function get(){return{patientInfo:{type:Array}}}}]);return MyRegister}(_myApp.PolymerElement);window.customElements.define("my-register",MyRegister)});